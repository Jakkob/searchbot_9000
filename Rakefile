# Set a global variable for the root directory for convenience
require 'pathname'
$ROOT_DIR = Pathname("#{File.expand_path('../', __FILE__)}")

require "#{$ROOT_DIR}/lib/init"

# task :default => :migrate

# desc "Run Migrations"
# task :migrate do
# 	ActiveRecord::Migrator.migrate("#{$ROOT_DIR}/db/migrate", ENV["VERSION"] ? ENV["VERSION"].to_i : nil)
# end
...
namespace :db do
  ...
  task :environment do
    DATABASE_ENV = ENV['DATABASE_ENV'] || 'main'
    MIGRATIONS_DIR = ENV['MIGRATIONS_DIR'] || 'db/migrate'
  end

  task :configuration => :environment do
  	template = ERB.new File.read("#{$ROOT_DIR}/config/database.yml")
  	@config = YAML.load(template.result(binding))[DATABASE_ENV]
  end
